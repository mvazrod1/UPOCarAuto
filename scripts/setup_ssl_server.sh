...
# Configuración
DOMAIN="upocarauto.sytes.net"
PASSWORD="Upo2025."  # debe ser una contraseña segura
...
GLASSFISH_DOMAIN_DIR="$GLASSFISH_HOME/glassfish/domains/domain1"
KEYSTORE_PATH="$GLASSFISH_DOMAIN_DIR/config/keystore.jks"
TRUSTSTORE_PATH="$GLASSFISH_DOMAIN_DIR/config/cacerts.jks"
KEYSTORE_PASSWORD="changeit" # Contraseña del keystore.jks de GlassFish. Por defecto es 'changeit' para dominios nuevos. ¡VERIFICA!

echo "--- Configuración de SSL/HTTPS para GlassFish ---"
echo "Dominio: $DOMAIN"
echo "Directorio SSL: $SSL_DIR"
echo "GlassFish HOME: $GLASSFISH_HOME"
echo "Directorio del Dominio: $GLASSFISH_DOMAIN_DIR" # Añadido para verificación en la salida
...
# Verificación del directorio del dominio con la ruta corregida
if [ ! -d "$GLASSFISH_DOMAIN_DIR" ]; then
    ...
    echo "Asegúrese de que la ruta '$GLASSFISH_DOMAIN_DIR' es correcta."
    exit 1
fi
...
# Usamos la contraseña del keystore.jks de GlassFish (-deststorepass)
# Añadimos -destkeypass para establecer explicitamente la contraseña de la entrada de la clave privada
# CAMBIO: Establecer la contraseña de la entrada de la clave privada a la misma que la del keystore de destino
...
if [ $? -ne 0 ]; then
    ...
    echo "Esto puede ocurrir si la contraseña del keystore de destino ('$KEYSTORE_PASSWORD') es incorrecta,"
    echo "o si el alias 'glassfish-cert' ya existe en $KEYSTORE_PATH y se usó -noprompt (intente eliminar el alias manualmente)."
    exit 1
fi
echo "Certificado importado correctamente al keystore de GlassFish con alias 'glassfish-cert'."


# Verificar si el dominio está accesible con asadmin; iniciar si no lo está
echo "Verificando accesibilidad del dominio GlassFish con asadmin..."

# Crear archivo de contraseña temporal para asadmin (necesario para list-domains si requiere auth)
PASSWORD_FILE=$(mktemp)
echo "$PASSWORD" > "$PASSWORD_FILE" # Asumimos que la contraseña de GlassFish Admin es la misma que la del certificado PKCS12. CAMBIA si es diferente.
chmod 600 "$PASSWORD_FILE" # Asegurar que solo el propietario puede leerlo

# Intentar un comando asadmin simple que requiere conexion (list-domains)
if ! "$GLASSFISH_HOME"/bin/asadmin --passwordfile "$PASSWORD_FILE" list-domains >/dev/null 2>&1; then
    echo "Dominio GlassFish no detectado como accesible via asadmin. Intentando iniciar con asadmin..."
    # Iniciar el dominio GlassFish
    "$GLASSFISH_HOME"/bin/asadmin start-domain domain1
    START_DOMAIN_EXIT_CODE=$?
    if [ $START_DOMAIN_EXIT_CODE -ne 0 ]; then
        echo "Error: Falló el inicio del dominio GlassFish 'domain1'."
        echo "El script no puede continuar con la configuración de GlassFish si el dominio no está corriendo."
        rm "$PASSWORD_FILE" # Limpiar el archivo de contraseña antes de salir
        exit 1
    fi
    echo "Dominio GlassFish iniciado correctamente con asadmin."
    # Dar un momento al dominio para que se inicialice completamente antes de enviar mas comandos
    echo "Esperando 10 segundos para que el dominio se inicialice..."
    sleep 10
else
    echo "Dominio GlassFish ya está corriendo y accesible via asadmin."
fi
...
# El archivo de contraseña temporal PASSWORD_FILE ya fue creado en la sección anterior

# Ahora el dominio debería estar corriendo y asadmin podrá conectarse
$GLASSFISH_HOME/bin/asadmin --passwordfile "$PASSWORD_FILE" << EOF

# Los comandos 'echo' y la sintaxis de shell '||' no funcionan dentro de asadmin.
# Los comentarios al final de la línea de un comando tampoco funcionan.

# Habilitar SSL y configurar protocolos para el listener HTTPS
# Eliminar la primera set ssl.enabled que fallaba en el log. security-enabled parece suficiente.
...
# Mostrar puerto actual (eliminar comentario al final de la linea)
get configs.config.server-config.network-config.network-listeners.network-listener.${HTTPS_LISTENER}.port
...
# CAMBIO: Usar disable-network-listener en lugar de disable
# CAMBIO: Añadir || true para ignorar el fallo de este comando si ocurre
disable-network-listener ${HTTP_LISTENER} || true

# Reiniciar dominio para aplicar cambios
# Eliminar el comando echo y el comentario al final de la linea
restart-domain --force

exit
EOF

# Limpiar archivo de contraseña temporal (ahora se crea antes del check)
# Si el script salió antes (por fallo en start-domain), ya se limpió.
...
echo "2. El listener HTTP (${HTTP_LISTENER}) ha sido deshabilitado. Solo se podrá acceder por HTTPS." # Esta nota será correcta SI disable-network-listener funcionó
...
echo "   Esta contraseña debe ser la contraseña REAL del archivo keystore.jks de GlassFish ($KEYSTORE_PATH)." # Nota ajustada
echo "6. Si el script falla en la importación con keytool, puede ser porque el alias 'glassfish-cert' ya existe."
echo "7. El dominio GlassFish fue iniciado por este script SI NO ESTABA ACCESIBLE via asadmin, y posteriormente reiniciado para aplicar los cambios." # Nota ajustada
echo "8. Si el listener HTTP ($HTTP_LISTENER) no se deshabilitó correctamente, podría ser necesario hacerlo manualmente." # Nueva nota de advertencia
# NOTA AJUSTADA: La contraseña de la entrada de clave se establece a la del keystore de destino
echo "9. La contraseña para la entrada de la clave privada 'glassfish-cert' en el keystore ha sido establecida a la misma que la contraseña del keystore destino ('$KEYSTORE_PASSWORD')."
...
# Puedes listar los certificados en el keystore con: keytool -list -v -keystore "$KEYSTORE_PATH" -storepass "$KEYSTORE_PASSWORD" -alias glassfish-cert -keypass "$KEYSTORE_PASSWORD" # Ejemplo para verificar la entrada de clave
